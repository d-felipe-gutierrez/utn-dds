...
async function DatosContratos() {
  try {
    const contratosPath = path.join(__dirname, '../datos_contratos.json');
    const data = fs.readFileSync(contratosPath, 'utf8');
    const contratosJson = await JSON.parse(data);

    await contratos.bulkCreate(contratosJson);
    console.log('Datos de contratos cargados exitosamente.');
  } catch (error) {
